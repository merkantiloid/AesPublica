    ...
    def parse(self):
        ...
        for item in items:
            ...
            if materials:
                ...
                db_item = MoonMatItem.query.filter(MoonMatItem.moon_mat_id == model.id,
                                                   MoonMatItem.type_id == item['type_id']).first()
                if not db_item:
        ...
        if len(ids) == 0:
            ...
            db.session.execute('delete from moon_mat_items where moon_mat_id = :id and type_id not in :ids',
                               params={'id': model.id, 'ids': ids})
        db.session.commit()
